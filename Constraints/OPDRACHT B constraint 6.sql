...
go
CREATE OR ALTER TRIGGER trainerNeedsExperience
ON offr
AFTER INSERT, UPDATE
AS
BEGIN
	...
	IF UPDATE(trainer)
		BEGIN
			...
			IF EXISTS(SELECT '' 
				FROM inserted i INNER JOIN emp e on i.trainer = e.empno 
				WHERE e.job != 'TRAINER'
				)
			...
			IF EXISTS(SELECT '' 
				FROM inserted i INNER JOIN reg r on i.trainer = r.stud 
...
--INSERT into offr met een niet trainer
BEGIN TRANSACTION
	INSERT INTO offr values 
	--fout
		('AM4DP', GETDATE(), 'CONF', 6, 1011, 'SAN FRANCISCO'),
	--goed
		('AM4DP', DATEADD(MONTH, -2, GETDATE()), 'CONF', 6, 1018, 'SAN FRANCISCO'),
	--fout
		('AM4DP', DATEADD(MONTH, -1, GETDATE()), 'CONF', 6, 1012, 'SAN FRANCISCO')
ROLLBACK TRAN

--INSERT into offr de opgegeven trainer is niet langgenoeg in dienst.
BEGIN TRANSACTION
	...
	--fout
		('AM4DP', GETDATE(), 'CONF', 6, 1011, 'SAN FRANCISCO'),
	--goed
		('AM4DP', DATEADD(MONTH, -2, GETDATE()), 'CONF', 6, 1013, 'SAN FRANCISCO'),
	--fout
		('AM4DP', DATEADD(MONTH, -1, GETDATE()), 'CONF', 6, 1012, 'SAN FRANCISCO')
ROLLBACK TRAN

--INSERT into offr de opgegeven trainer heeft de course niet gevolgd.
BEGIN TRANSACTION
